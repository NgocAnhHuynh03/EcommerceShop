...
(function ($) {
  ...
  var methods = {
    ...
    show: function show(el, ordinal) {
      ...
      lastclicked[el.id] = ordinal; // Add popup id to visiblePopupsArray
      ...
      $el.data('focusedelementbeforepopup', document.activeElement); // Make holder div programatically focusable with tabindex:-1 (tabindex:0 is keyboard focusable)

      $el.attr('tabindex', -1); // Focus the popup or user specified element.
      ...
      }, options.focusdelay); // Hide main content from screen readers
    ...
    hide: function hide(el, outerClick) {
      ...
      var popupIdIndex = $.inArray(el.id, visiblePopupsArray); // If popup is not opened, ignore the rest of the function
  ...
  $(document).on('click', function (event) {
    if (visiblePopupsArray.length) {
      var elementId = visiblePopupsArray[visiblePopupsArray.length - 1];
      var el = document.getElementById(elementId);
      var closeButton = $(el).data('popupoptions') && $(el).data('popupoptions').closeelement ? $(el).data('popupoptions').closeelement : '.' + el.id + closesuffix; // If Close button clicked
      ...
      if ($(el).data('popupoptions') && $(el).data('popupoptions').blur && !$(event.target).closest($(el).data('popupoptions').blurignore).length && !$(event.target).closest('#' + elementId).length && !$(mousedownTarget).closest('#' + elementId).length && event.which !== 2 && $(event.target).is(':visible')) {
        if ($(el).data('popupoptions').background) {
          ...
          methods.hide(el); // Older iOS/Safari will trigger a click on the elements below the cover,
  ...
  $(document).on('keydown', function (event) {
    if (visiblePopupsArray.length && event.which == 9) {
      ...
      var elementId = visiblePopupsArray[visiblePopupsArray.length - 1];
      var el = document.getElementById(elementId);
      var options = $(el).data('popupoptions'); // If the last opened popup doesn't have `keepfocus` option, ignore the rest and don't lock the focus inside of popup.
      ...
      var popupItems = $(el).find('*'); // Get list of focusable items

      var focusableItems = popupItems.filter(focusableElementsString).filter(':visible'); // Get currently focused item

      var focusedItem = $(':focus'); // Get the number of focusable items

      var numberOfFocusableItems = focusableItems.length; // Get the index of the currently focused item

      var focusedItemIndex = focusableItems.index(focusedItem); // If popup doesn't contain focusable elements, focus popup itself
  ...
  $.fn.popup = function (customoptions) {
    return this.each(function () {
      var $el = $(this);
      var newDefaults = $.extend(true, {}, $.fn.popup.defaults); // Set defaults for tooltips dynamically instead of implicitly, so they can be overriden with custom options.
  ...
  $.fn.popup.destroyall = function () {
    ...
    $('.popup_background').remove(); // visiblePopupsArray = []; // TODO: check if we need this for SPA and popups with fadeOut animation and scrolllock
