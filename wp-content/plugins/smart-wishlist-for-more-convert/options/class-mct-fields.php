					...
					<div class="wrap mct-wrapper">
						<?php if ( isset( $this->title ) ) : ?>
							<div class="mct-header">
								<figure class="d-flex margin-bet f-center no-mar">
											...
											<?php if ( ( empty( $this->sections ) ) && false === $type_class ) : ?>
												<div class="d-flex space-between mt-20">
													...
													<button class="btn-secondary min-width-btn  mct-reset-btn"
															...
															value="<?php echo esc_attr( $k ); ?>"
															type="submit"><?php esc_html_e( 'Reset Settings', 'mct-options' ); ?></button>
													<button class="btn-primary min-width-btn ico-btn save-btn btn-fixed-position mct-save-btn" name="mct-action" value="<?php echo esc_attr( $k ); ?>" type="submit">
											...
											<div class="mct-sidebar-inner">
												<?php foreach ( $this->sidebar as $sidebar ) : ?>
													<div class="mct-article">
														...
														<?php if ( isset( $sidebar['button'] ) && ! empty( $sidebar['button'] ) ) : ?>
															<p>
																<a target="<?php echo isset( $sidebar['button']['btn_target'] ) ? esc_attr( $sidebar['button']['btn_target'] ) : '_blank'; ?>" class="<?php echo esc_attr( $sidebar['button']['btn_class'] ); ?>" href="<?php echo esc_url( $sidebar['button']['btn_url'] ); ?>"><?php echo esc_attr( $sidebar['button']['btn_label'] ); ?></a>
															</p>
														<?php endif; ?>
				...
				<?php if ( 'wizard-type' === $this->type && ! empty( $this->steps ) ) : ?>
					...
					<div class="wrap mct-wrapper mct-wizard">
						...
						<div class="mct-options ">
									...
									<div id="wizard_content_<?php echo esc_attr( $k ); ?>" class="wizard-content mct-article <?php echo esc_attr( $k ); ?>" <?php echo ( $current_step !== $k ) ? 'style="display:none"' : ''; ?> >
										...
										<?php if ( 'welcome' === $k ) : ?>
											...
											<p class="center-align">
												<a href="<?php echo esc_url( add_query_arg( 'mct-' . $this->id . '-wizard-skip', '1', admin_url() ) ); ?>" class="btn-text"><?php esc_attr_e( 'NO, Not Ready Now', 'mct-options' ); ?></a>
											</p>
										<?php elseif ( 'ready' === $k ) : ?>
											<?php // do anything. ?>
										<?php else : ?>
							...
							<p class="center-align">
								<a class="btn-primary brown-btn " href="<?php echo 'ready' === $current_step ? esc_url( admin_url() ) : esc_url( add_query_arg( 'mct-' . $this->id . '-wizard-skip', '1', admin_url() ) ); ?>"><?php esc_attr_e( 'Back to WordPress Dashboard', 'mct-options' ); ?></a>
							</p>
                            <?php endif;?>
						</div>
					</div>
									...
									<div id="wizard_content_<?php echo esc_attr( $k ); ?>" class="wizard-content mct-article <?php echo esc_attr( $k ); ?>" <?php echo ( $current_step !== $k ) ? 'style="display:none"' : ''; ?> >
										<div class="d-flex f-center space-between">
											...
											<div class="d-flex f-center gap-5">
												<?php if ( isset( $step['help'] ) || isset( $step['youtube'] ) || isset( $step['doc'] ) ) : ?>
													...
													<div class="mct-help-tip-wrap no-float">
														<span class="mct-help-tip-dec">
															<?php if ( isset( $step['help_image'] ) && ! empty( $step['help_image'] ) ) : ?>
																<img src="<?php echo esc_url( $step['help_image'] ); ?>" alt="<?php echo esc_attr( $step['title'] ); ?>"/>
															<?php endif; ?>
															<p><?php echo wp_kses_post( $step['help'] ); ?></p>
														</span>
													</div>
												<?php endif; ?>
												<?php if ( isset( $step['youtube'] ) ) : ?>
													<!-- MCT YouTube -->
													<a href="<?php echo esc_url( $step['youtube'] ); ?>" target="_blank" class="mct-youtube-url">
														<span class="dashicons dashicons-youtube"></span>
													</a>
												<?php endif; ?>
												<?php if ( isset( $step['doc'] ) ) : ?>
													<!-- MCT Document -->
													<a href="<?php echo esc_url( $step['doc'] ); ?>" target="_blank" class="mct-doc-url ">
		...
		public function print_table_fields( $section_key, $fields ) {
			...
			<table class="form-table" role="presentation">
				...
				<?php foreach ( $fields as $fieldkey => $field ) : ?>
					...
					<?php if ( ! in_array( $field['type'], array( 'end', 'separator', 'start', 'manage', 'iframe', 'column-start', 'column-end', 'columns-start', 'columns-end' ), true ) ) : ?>
					...
					if ( isset( $value['tabs'] ) ) {
						foreach ( $value['tabs'] as $tab => $fields ) {
							foreach ( $this->options[ $section ]['fields'][ $tab ] as $k => $v ) {
								if ( isset( $v['type'] ) && ! in_array( $v['type'], array( 'end', 'separator', 'start', 'iframe', 'column-start', 'column-end', 'columns-start', 'columns-end' ), true ) ) {
